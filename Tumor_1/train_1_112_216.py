...
def train_net(net, train_txt, save_train_dict, lr=0.001, batch_size=100, epochs=50):
    ...
    scheduler = CosineAnnealingWarmRestarts(optimizer,T_0=3,T_mult=2) #3 22/46 5 36/76
    # 定义Loss算法
    ...
    # train
        ...
        with tqdm(total=len(train_dataset), desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for ct, pet, doctor_glod, data_list  in train_dataLoader:
                ...
                if epoch<3:
                    loss = 0.001*loss_up + (loss_pre + loss_doc)
                else:
...
if __name__ == "__main__":
    ...
    print('!!!Train!!!')
    print("##################################################################")
    train_txt = './order/train_1_216_112.txt'
    save_train_dict = './params/1_216_112'
    train_net(net, train_txt, save_train_dict)

    print('!!!Predict!!!')
    model_dir_all = './params/1_216_112'
    train_txt = './order/test_1_216_112.txt'
